...
- With the lunar limb profile installed, the C2 and C3 rows show the contact-point corrected times printed by Solar Eclipse Maestro.  Two extra rows show a photographic Baily's beads capture window around each internal contact.  Its inner edges, `BEADS_C2_END` and `BEADS_C3_START`, are the distinct whole-arc instants when the last bead vanishes and the first bead returns.  The checkbox below the table switches the correction off if you would rather see the mean-limb times; it is locked while a script has pending jobs so displayed contacts cannot disagree with already-scheduled shutters.
- The corrected contacts are what a script schedules against: `C2` and `C3` are the contact-point corrected times, `C2_MEAN` and `C3_MEAN` the uncorrected ones, and `BEADS_C2` / `BEADS_C3` (with `_START` and `_END`) the capture windows.  A window is published only when both edges were solved; an unresolved search produces no schedulable bead timestamps.
- The capture-window boundary is a photographic heuristic: by default it is where no more than 20° total of the lunar limb remains illuminated.  The solver's `arc_degrees` argument makes that threshold configurable; measurements around the reference cases moved the outer edge by about 0.18 s per degree.
- The limb profile itself is a 72 MB file, `lunar_limb_band_v1.bin`, which is not part of the checkout.  Put it next to the ephemeris (the directory Solar Eclipse Workbench is started from) to enable the correction; it is built by `tools/build_limb_blob.py` from **LOLA LDEM_128** (`LRO-L-LOLA-4-GDR-V1.0`).  Kaguya is relevant only to the separate, undocumented LRO/Kaguya profile blend used by Solar Eclipse Maestro.  Without the blob everything still works with mean-limb contacts, and the bead rows say "no limb profile".
- Contact geometry is top-of-atmosphere: atmospheric refraction is deliberately excluded.  At a solar altitude near 7.5°, its estimated effect is about 0.30 s at C2 and 0.22 s at C3.  Evaluating the lunar profile once at maximum instead of separately at C2/C3 changed four measured contacts on two eclipses by no more than 0.014 s.
- Published-profile validation has a data-source floor.  LOLA and Maestro's undocumented LRO/Kaguya blend differ by at least about 0.45 s at sampled contacts and by up to 1.25 s in the current three-sheet set, so the Maestro acceptance tolerance is 1.5 s; tighter assertions would measure source disagreement rather than solver precision.
...
- **take_burst** - Set the aperture, shutter speed and ISO of the camera and take a burst of pictures during 3 seconds (for Sony and modern Canon cameras; Nikon and older Canon cameras will take 3 pictures in burst mode instead). Note - for Sony cameras the automatically chosen Continuous mode might be either High Speed or Low Speed (completely depends on the cameras's model). If you want to define a specific mode - check out the `--sony-cont-mode` command line option. As for Nikon cameras - your Continuous mode might be dictated by the physical dial. In such a case, please set it beforehand accordingly (it won't affect `take_picture` or `take_hdr`, so no need to touch it once set)!

```take_burst, C1, +, 0:00:08.0, Canon EOS 80D, 1/2000, 5.6, 400, 3, "Burst test"```

- **take_bracket** - Set the aperture, shutter speed and ISO of the camera and take a bracket of 5 pictures with the given steps. Make sure to have 5 steps enabled for bracketing (note - if not supported, it will fallback to 3 steps instead). Options for the steps are: +/- 1/3, +/- 2/3, +/- 1, +/- 1 1/3, +/- 1 2/3, +/- 2, +/- 2 1/3, +/- 2 2/3, +/- 3.
This syntax only works for Canon. Sony doesn't support it, so instead you have to provide the desired mode name in this field instead (e.g. `"Bracketing C 3.0 Steps 5 Pictures"`). You can look up for the available modes using this command: `gphoto2  --get-config=/main/capturesettings/capturemode`.
As for Nikon - it also doesn't support this syntax, so please use instead this convention: `"0.7 5"`. First float number is exposure step in EV and the other number is the amount of frames to take in the bracketing sequence.  An important note for Nikon cameras with physical dial for capture mode - app will assume you already chose Continuous mode (please see `take_burst` above for more information).

```take_bracket, C1, +, 0:00:08.0, Canon EOS 80D, 1/2000, 5.6, 400, "+/- 1 2/3", "Bracket test"```

- **take_hdr** - Take an HDR sequence by ramping the shutter speed from a starting (fastest) speed down by the given number of full stops and back up again, while keeping aperture and ISO fixed.  Uses `gp_camera_trigger_capture` for maximum speed so successive shots are fired without waiting for each file to be written to the card. The shutter speed choices available on the connected camera are queried at runtime, so the sequence always stays within the actual speeds the body supports. Total shots fired: 2 × stops + 1 (the slowest exposure appears once at the midpoint). An important warning for Sony cameras - if the camera is in `PC` mode, the total number of taken shots must not exceed the size of camera's buffer! Otherwise, the entire sequence will be lost. You can find the buffer size of some of the Sony cameras [here](https://capturetheuniverse.com/#buffer-size-usb-speed).
