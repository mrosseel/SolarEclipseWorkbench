...
def test_fuji_sdk(count: int, sdk_path: str):
    from fujixsdk import (
        ...
        SHUTTER_1_8000, ISO_100,
        SHUTTER_SPEED_NAMES, DRIVE_MODE_NAMES, DRIVE_MODE_S, DRIVE_MODE_MOVIE,
    )
    ...
    with Camera(sdk_path, cameras[0].device_name) as cam:
        ...
        for attempt in range(20):
            try:
                ...
                time.sleep(0.5)
        if not priority_set:
            print("ERROR: Could not set PC priority after 20 attempts")
            return
        ...
        if drained:
            print(f"Drained {drained} pending image(s)")
            time.sleep(0.5)

        try:
            # Configure — camera may still be busy after drain/priority change
            ...
            # Drive mode: SDK can only read S/MOVIE/INVALID on dial-equipped cameras.
            # SetDriveMode cannot set CH — the user must set it on the physical dial.
            dm = cam.get_drive_mode()
            dm_name = DRIVE_MODE_NAMES.get(dm, f"0x{dm:04X}")
            if dm == DRIVE_MODE_MOVIE:
                print(f"ERROR: Camera is in MOVIE mode (drive dial). Set dial to CH and retry.")
                return
            if dm != DRIVE_MODE_S:
                print(f"WARNING: Unexpected drive mode {dm_name} (0x{dm:04X})")
            print(f"Drive mode: {dm_name} (set CH on the camera dial for burst)")

            speed, _ = cam.get_shutter_speed()
            speed_name = SHUTTER_SPEED_NAMES.get(speed, f"? ({speed})")
            captured, total = cam.get_buffer_capacity()
            available = total - captured
            print(f"Shutter: {speed_name}  ISO: {cam.get_iso()}  "
                  f"Aperture: f/{cam.get_aperture() / 100:.1f}")
            print(f"Buffer: {captured}/{total} captured, {available} available")
            print()

            if available < count:
                print(f"WARNING: buffer only has room for {available} shots, "
            ...
            captured2, total2 = cam.get_buffer_capacity()
            available2 = total2 - captured2
            count2 = min(count, available2)
            ...
            for i in range(count2):
                ...
                cap, tot = cam.get_buffer_capacity()
                if cap >= tot:
                ...
                for retry in range(5):
                    ...
                    print(f"  Shot {i} failed after retries")
                    break
            ...
            # Always return control to camera, even on error
            try:
...
def main():
    ...
    if run_gp:
        try:
            gp_count = args.count if args.gphoto2 else GPHOTO2_SHOT_COUNT
            test_gphoto2(gp_count)
        except ImportError:
